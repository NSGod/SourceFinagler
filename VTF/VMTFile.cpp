...
vlBool CVMTFile::Load(const vlChar *cFileName)
{
	...
	vlBool result = this->Load(&reader);
	return result;
}

vlBool CVMTFile::Load(const vlVoid *lpData, vlUInt uiBufferSize)
{
	IO::Readers::CMemoryReader memReader = IO::Readers::CMemoryReader(lpData, uiBufferSize);
	vlBool result = this->Load(&memReader);
	return result;
}

vlBool CVMTFile::Load(vlVoid *pUserData)
{
	IO::Readers::CProcReader procReader = IO::Readers::CProcReader(pUserData);
	vlBool result = this->Load(&procReader);
	return result;
}

vlBool CVMTFile::Save(const vlChar *cFileName) const
{
	IO::Writers::CFileWriter fileWriter = IO::Writers::CFileWriter(cFileName);
	vlBool result = this->Save(&fileWriter);
	return result;
}
...
{
	IO::Writers::CProcWriter procWriter = IO::Writers::CProcWriter(pUserData);
	vlBool result = this->Save(&procWriter);
	return result;
}
