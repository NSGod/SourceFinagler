...
#	include <VTFNVTT/stdafx.h>
#	include <VTFNVTT/Readers.h>
#	include <VTFNVTT/Writers.h>
...
namespace VTFLib {
	...
	*/
	...
	private:
		...
		vlUInt			imageDataLength;				// Size of VTF image data buffer
		vlByte			*imageData;					// VTF image buffer

		vlUInt			thumbnailImageDataLength;			// Size of VTF thumbnail image data buffer
		vlByte			*thumbnailImageData;			// VTF thumbnail image buffer
	...
	public:
		...
		/*!
			Checks to see if the resource type exists and returns its data.
	...
	public:
		...
		*/
		vlBool GenerateMipmaps(VTFMipmapFilter MipmapFilter = MIPMAP_FILTER_BOX, VTFSharpenFilter SharpenFilter = SHARPEN_FILTER_DEFAULT);
		...
		/*!
			...
			\return true on sucessful creation, otherwise false.
		*/
		vlBool GenerateMipmaps(vlUInt uiFace, vlUInt uiFrame, VTFMipmapFilter MipmapFilter = MIPMAP_FILTER_BOX, VTFSharpenFilter SharpenFilter = SHARPEN_FILTER_DEFAULT);
		...
		*/
		vlBool GenerateNormalMap(VTFKernelFilter KernelFilter = KERNEL_FILTER_3X3, VTFHeightConversionMethod HeightConversionMethod = HEIGHT_CONVERSION_METHOD_DEFAULT, VTFNormalAlphaResult NormalAlphaResult = NORMAL_ALPHA_RESULT_DEFAULT);
		...
		/*!
			...
			\note  The options for conversion are the same used in the nVidea NormalMap Photoshop plug-in.
		*/
		vlBool GenerateNormalMap(vlUInt uiFrame, VTFKernelFilter KernelFilter = KERNEL_FILTER_3X3, VTFHeightConversionMethod HeightConversionMethod = HEIGHT_CONVERSION_METHOD_DEFAULT, VTFNormalAlphaResult NormalAlphaResult = NORMAL_ALPHA_RESULT_DEFAULT);
...
#pragma mark static (Class) methods
	...
	public:
		...
		*/
		static vlBool Resize(vlByte *lpSourceRGBA8888, vlByte *lpDestRGBA8888, vlUInt uiSourceWidth, vlUInt uiSourceHeight, vlUInt uiDestWidth, vlUInt uiDestHeight, VTFMipmapFilter ResizeFilter = MIPMAP_FILTER_TRIANGLE, VTFSharpenFilter SharpenFilter = SHARPEN_FILTER_DEFAULT);
