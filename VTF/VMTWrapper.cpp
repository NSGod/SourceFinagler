...
VTFLIB_API vlBool vlBindMaterial(vlUInt uiMaterial) {
	...
	if (uiMaterial >= lpMaterialVector->size() || (*lpMaterialVector)[uiMaterial] == 0) {
		LastError.Set("Invalid material.");
		return vlFalse;
	}
	if (lpMaterial == (*lpMaterialVector)[uiMaterial]) { // If it is already bound do nothing.
		return vlTrue;
	}
	lpMaterial = (*lpMaterialVector)[uiMaterial];
...
VTFLIB_API vlBool vlCreateMaterial(vlUInt *uiMaterial) {
	...
	lpMaterialVector->push_back(new CVMTFile());
	*uiMaterial = (vlUInt)lpMaterialVector->size() - 1;
...
VTFLIB_API vlVoid vlDeleteMaterial(vlUInt uiMaterial) {
	...
	if (uiMaterial >= lpMaterialVector->size()) {
		return;
	}
	if ((*lpMaterialVector)[uiMaterial] == 0) {
		return;
	}
	if ((*lpMaterialVector)[uiMaterial] == lpMaterial) {
		lpMaterial = 0;

		CurrentIndex.clear();
		CurrentNode = 0;
	}
	delete (*lpMaterialVector)[uiMaterial];
	(*lpMaterialVector)[uiMaterial] = 0;
}
...
VTFLIB_API vlBool vlMaterialCreate(const vlChar *cRoot) {
	...
	return lpMaterial->Create(cRoot);
}
...
VTFLIB_API vlBool vlMaterialLoad(const vlChar *cFileName) {
	...
	return lpMaterial->Load(cFileName);
}
...
VTFLIB_API vlBool vlMaterialLoadLump(const vlVoid *lpData, vlUInt uiBufferSize) {
	...
	return lpMaterial->Load(lpData, uiBufferSize);
}
...
VTFLIB_API vlBool vlMaterialLoadProc(vlVoid *pUserData) {
	...
	return lpMaterial->Load(pUserData);
}
...
VTFLIB_API vlBool vlMaterialSave(const vlChar *cFileName) {
	...
	return lpMaterial->Save(cFileName);
}
...
VTFLIB_API vlBool vlMaterialSaveLump(vlVoid *lpData, vlUInt uiBufferSize, vlUInt *uiSize) {
	...
	return lpMaterial->Save(lpData, uiBufferSize, *uiSize);
}
...
VTFLIB_API vlBool vlMaterialSaveProc(vlVoid *pUserData) {
	...
	return lpMaterial->Save(pUserData);
}
...
VTFLIB_API vlBool vlMaterialGetFirstNode() {
	if (lpMaterial == 0 || lpMaterial->GetRoot() == 0) {
		return vlFalse;
	}
	CurrentNode = lpMaterial->GetRoot();
