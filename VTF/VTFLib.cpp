...
namespace VTFLib {
	...
	CVTFFile *lpImage = 0;
	CImageVector *lpImageVector = 0;
	
	CVMTFile *lpMaterial = 0;
	CMaterialVector *lpMaterialVector = 0;
	
	vlUInt uiDXTQuality = DXT_QUALITY_HIGHEST;
...
VTFLIB_API vlBool vlInitialize() {
	...
	lpImageVector = new CImageVector();
	lpMaterialVector = new CMaterialVector();
...
VTFLIB_API vlVoid vlShutdown() {
	...
	lpImage = 0;
	lpMaterial = 0;
	for (i = 0; i < lpImageVector->size(); i++) {
		delete (*lpImageVector)[i];
	}

	delete lpImageVector;
	lpImageVector = 0;
	for (i = 0; i < lpMaterialVector->size(); i++) {
		delete (*lpMaterialVector)[i];
	}

	delete lpMaterialVector;
	lpMaterialVector = 0;
}
