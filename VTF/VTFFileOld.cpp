...
#	include <VTFNVTT/VTFLib.h>
#	include <VTFNVTT/VTFFile.h>
#	include <VTFNVTT/VTFFormat.h>
#	include <VTFNVTT/VTFDXTn.h>
#	include <VTFNVTT/VTFMathlib.h>
#else
...
public:
	...
	SNVCompressionUserData(CVTFFile *pVTFFile, vlUInt uiFrame, vlUInt uiFace, vlUInt uiSlice, VTFImageFormat ImageFormat) :
		lpData(0), pVTFFile(pVTFFile), uiFrame(uiFrame), uiFace(uiFace), uiSlice(uiSlice), ImageFormat(ImageFormat) {
...
NV_ERROR_CODE NVWriteCallback(const void *buffer, size_t count, const MIPMapData *mipMapData, void *userData) {
	...
	if (UserData->pVTFFile != 0) {
		...
		if (UserData->ImageFormat == UserData->pVTFFile->GetFormat()) {
			...
			assert(UserData->pVTFFile->GetFormat() != IMAGE_FORMAT_DXT1 &&
				   UserData->pVTFFile->GetFormat() != IMAGE_FORMAT_DXT1_ONEBITALPHA &&
				   UserData->pVTFFile->GetFormat() != IMAGE_FORMAT_DXT3 &&
				   UserData->pVTFFile->GetFormat() != IMAGE_FORMAT_DXT5);
...
struct NVOutputHandler : public OutputHandler {
		...
		if (this->currentOffset + aSize <= this->currentLength) {
			...
			if (this->currentOffset == this->currentLength) {
				if (this->userData->pVTFFile != 0) {
					printf("size == %d, computedImageSize == %u\n\n", aSize, CVTFFile::ComputeImageSize(this->mipmapWidth, this->mipmapHeight, this->userData->uiSlice, this->userData->ImageFormat));
					...
					if (this->userData->pVTFFile->GetFormat() == this->userData->ImageFormat) {
						...
						CVTFFile::ConvertFromRGBA8888(this->currentData,
													  ...
													  this->mipmapWidth,
													  this->mipmapHeight,
													  this->userData->pVTFFile->GetFormat());
...
CVTFFile::CVTFFile() {
	...
	this->thumbnailImageDataLength = 0;
	this->thumbnailImageData = 0;
}
...
CVTFFile::CVTFFile(const CVTFFile &VTFFile) {
	...
	if (VTFFile.IsLoaded()) {
		...
		if (VTFFile.GetHasImage()) {
			this->imageDataLength = VTFFile.imageDataLength;
			this->imageData = new vlByte[this->imageDataLength];
			memcpy(this->imageData, VTFFile.imageData, this->imageDataLength);
		}
		if (VTFFile.GetHasThumbnail()) {
			this->thumbnailImageDataLength = VTFFile.thumbnailImageDataLength;
			this->thumbnailImageData = new vlByte[this->thumbnailImageDataLength];
			memcpy(this->thumbnailImageData, VTFFile.thumbnailImageData, this->thumbnailImageDataLength);
		}
	}
}
...
CVTFFile::CVTFFile(const CVTFFile &VTFFile, VTFImageFormat ImageFormat) {
	...
	if (VTFFile.IsLoaded()) {
		...
		if (VTFFile.GetHasImage()) {
			...
			this->imageDataLength = this->ComputeImageSize(this->header->Width, this->header->Height, uiMipmaps, this->header->ImageFormat) * uiFrames * uiFaces;
			this->imageData = new vlByte[this->imageDataLength];

			//vlByte *imageData = new vlByte[this->ComputeImageSize(this->header->Width, this->header->Height, 1, IMAGE_FORMAT_RGBA8888)];
				...
				for (vlUInt j = 0; j < uiFaces; j++) {
					for (vlUInt k = 0; k < uiSlices; k++) {
						for (vlUInt l = 0; l < uiMipmaps; l++) {
							...
							//this->ConvertToRGBA8888(VTFFile.GetData(i, j, k, l), imageData, uiMipmapWidth, uiMipmapHeight, VTFFile.GetFormat());
							//this->ConvertFromRGBA8888(imageData, this->GetData(i, j, k, l), uiMipmapWidth, uiMipmapHeight, this->GetFormat());
...
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices, VTFImageFormat ImageFormat, vlBool bThumbnail, vlBool bMipmaps, vlBool bNullImageData) {
	...
	if (bThumbnail) {
		...
		this->thumbnailImageDataLength = this->ComputeImageSize(this->header->LowResImageWidth, this->header->LowResImageHeight, 1, this->header->LowResImageFormat);
		this->thumbnailImageData = new vlByte[this->thumbnailImageDataLength];
	...
	this->imageDataLength = this->ComputeImageSize(this->header->Width, this->header->Height, this->header->Depth, this->header->MipCount, this->header->ImageFormat) * uiFrames * uiFaces;
	this->imageData = new vlByte[this->imageDataLength];
	...
	if (bNullImageData) {
		memset(this->thumbnailImageData, 0, this->thumbnailImageDataLength);
		memset(this->imageData, 0, this->imageDataLength);
	}
...
//			}
				...
				for (vlUInt j = 0; j < uiFaces; j++) {
					for (vlUInt k = 0; k < uiSlices; k++) {
						if (!this->ConvertToNormalMap(lpImageDataRGBA8888[i + j + k], 0 /*lpImageDataNormalMap*/,
													  ...
													  VTFCreateOptions.bNormalMinimumZ,
													  VTFCreateOptions.sNormalScale,
													  VTFCreateOptions.bNormalWrap,
													  VTFCreateOptions.bNormalInvertX,
		...
		if (VTFCreateOptions.bMipmaps && this->header->MipCount != 1) {
			...
			// Two loops will iterate only once.
			for (vlUInt frameIndex = 0; frameIndex < uiFrames; frameIndex++) {
				for (vlUInt faceIndex = 0; faceIndex < uiFaces; faceIndex++) {
					for (vlUInt sliceIndex = 0; sliceIndex < uiSlices; sliceIndex++) {
						...
						vlByte *rgbaBytes = lpImageDataRGBA8888[frameIndex + faceIndex + sliceIndex];
...
//						inputOptions.setMipmapGeneration(false);
						...
						switch (uiDXTQuality) {
							case DXT_QUALITY_LOW:		compressionOptions.setQuality(Quality_Fastest); break;
							case DXT_QUALITY_MEDIUM:	compressionOptions.setQuality(Quality_Normal); break;
							case DXT_QUALITY_HIGH:		compressionOptions.setQuality(Quality_Production); break;
							case DXT_QUALITY_HIGHEST:	compressionOptions.setQuality(Quality_Highest); break;
							default:					compressionOptions.setQuality(Quality_Production); break;
						}
...
vlVoid CVTFFile::Destroy() {
	...
	this->thumbnailImageDataLength = 0;
	delete [] this->thumbnailImageData;
	this->thumbnailImageData = 0;
}
...
vlBool CVTFFile::Load(const vlVoid *lpData, vlUInt uiBufferSize, vlBool bHeaderOnly) {
	IO::Readers::CMemoryReader reader = IO::Readers::CMemoryReader(lpData, uiBufferSize);
	vlBool bResult = this->Load(&reader, bHeaderOnly);
	return bResult;
}
...
vlBool CVTFFile::Save(const vlChar *cFileName) const {
	IO::Writers::CFileWriter writer = IO::Writers::CFileWriter(cFileName);
	vlBool bResult = this->Save(&writer);
	return bResult;
}
...
vlBool CVTFFile::Save(vlVoid *pUserData) const {
	IO::Writers::CProcWriter writer = IO::Writers::CProcWriter(pUserData);
	vlBool bResult = this->Save(&writer);
	return bResult;
}
...
vlBool CVTFFile::Load(IO::Readers::IReader *Reader, vlBool bHeaderOnly) {
	...
	try {
		...
		if (uiFileSize < sizeof(SVTFFileHeader)) {
			LastError.Set("File is corrupt; file too small for its header.");
			throw 0;
		}
...
//#endif
		...
		this->imageDataLength = this->ComputeImageSize(this->header->Width, this->header->Height, this->header->Depth, this->header->MipCount, this->header->ImageFormat) * this->GetFaceCount() * this->GetFrameCount();
...
//							Reader->Seek(this->header->Resources[i].Data, FILE_BEGIN);
			...
			uiImageDataOffset = uiThumbnailBufferOffset + this->thumbnailImageDataLength;
		}
		// sanity check
		// headersize + lowbuffersize + buffersize *should* equal the filesize
		if (this->header->HeaderSize > uiFileSize || uiThumbnailBufferOffset + this->thumbnailImageDataLength > uiFileSize || uiImageDataOffset + this->imageDataLength > uiFileSize) {
		...
		if (this->header->LowResImageFormat != IMAGE_FORMAT_NONE) {
			...
			if (Reader->Read(this->thumbnailImageData, this->thumbnailImageDataLength) != this->thumbnailImageDataLength) {
				throw 0;
			}
		}
		if (uiImageDataOffset == 0) {
		...
		if (this->header->ImageFormat != IMAGE_FORMAT_NONE) {
			...
			if (Reader->Read(this->imageData, this->imageDataLength) != this->imageDataLength) {
				throw 0;
			}
		}
	...
	try {
		...
		if (this->GetSupportsResources()) {
			for (vlUInt i = 0; i < this->header->ResourceCount; i++) {
				switch (this->header->Resources[i].Type) {
					case VTF_LEGACY_RSRC_LOW_RES_IMAGE:
						if (Writer->Write(this->thumbnailImageData, this->thumbnailImageDataLength) != this->thumbnailImageDataLength) {
							throw 0;
						}
						break;
					case VTF_LEGACY_RSRC_IMAGE:
						if (Writer->Write(this->imageData, this->imageDataLength) != this->imageDataLength) {
							throw 0;
						}
						break;
					default:
...
//
// GetHasImage()
// A image can be loaded as header only, this function indicates whether
	...
	for (vlUInt i = 0; i < this->header->ResourceCount; i++) {
		switch (this->header->Resources[i].Type) {
			case VTF_LEGACY_RSRC_LOW_RES_IMAGE:
				...
				uiOffset += this->thumbnailImageDataLength;
				break;
			case VTF_LEGACY_RSRC_IMAGE:
				this->header->Resources[i].Data = uiOffset;
				uiOffset += this->imageDataLength;
				break;
			default:
				if ((this->header->Resources[i].Flags & RSRCF_HAS_NO_DATA_CHUNK) == 0) {
					this->header->Resources[i].Data = uiOffset;
...
vlUInt CVTFFile::GetSize() const {
	...
	return this->header->HeaderSize + this->thumbnailImageDataLength + this->imageDataLength + uiResourceSize;
}
...
vlByte *CVTFFile::GetData(vlUInt uiFrame, vlUInt uiFace, vlUInt uiSlice, vlUInt uiMipmapLevel) const {
	...
	return this->imageData + this->ComputeDataOffset(uiFrame, uiFace, uiSlice, uiMipmapLevel, this->header->ImageFormat);
}
...
vlVoid CVTFFile::SetData(vlUInt uiFrame, vlUInt uiFace, vlUInt uiSlice, vlUInt uiMipmapLevel, vlByte *lpData) {
	if (!this->IsLoaded() || this->imageData == 0) {
		return;
	}
	memcpy(this->imageData + this->ComputeDataOffset(uiFrame, uiFace, uiSlice, uiMipmapLevel, this->header->ImageFormat), lpData, CVTFFile::ComputeMipmapSize(this->header->Width, this->header->Height, 1, uiMipmapLevel, this->header->ImageFormat));
}
...
vlVoid CVTFFile::SetThumbnailData(vlByte *lpData) {
	if (!this->IsLoaded() || this->thumbnailImageData == 0) {
		return;
	}
	memcpy(this->thumbnailImageData, lpData, this->thumbnailImageDataLength /*CVTFFile::ComputeImageSize(this->header->LowResImageWidth, this->header->LowResImageHeight, this->header->LowResImageFormat)*/);
}
...
vlVoid *CVTFFile::GetResourceData(vlUInt uiType, vlUInt &uiSize) const {
	if (this->IsLoaded()) {
		if (this->GetSupportsResources()) {
			switch (uiType) {
				case VTF_LEGACY_RSRC_LOW_RES_IMAGE:
					uiSize = this->thumbnailImageDataLength;
					return this->thumbnailImageData;
					break;
				case VTF_LEGACY_RSRC_IMAGE:
					uiSize = this->imageDataLength;
...
vlBool CVTFFile::GenerateMipmaps(vlUInt uiFace, vlUInt uiFrame, VTFMipmapFilter mipmapFilter, VTFSharpenFilter SharpenFilter) {
	printf("CVTFFile::GenerateMipmaps()\n");
	if (!this->IsLoaded()) {
		return vlFalse;
	}
...
//	vlByte *imageData = new vlByte[this->ComputeImageSize(this->header->Width, this->header->Height, 1, IMAGE_FORMAT_RGBA8888)];
	...
	if (!nvDXTCompressWrapper(theImageData, this->header->Width, this->header->Height, &Options, NVWriteCallback)) {
		delete [] theImageData;
...
//	inputOptions.setMipmapGeneration(false);
	inputOptions.setNormalizeMipmaps(true);
	inputOptions.setMipmapFilter((MipmapFilter)mipmapFilter);
...
vlBool CVTFFile::GenerateThumbnail() {
	...
	vlByte *theImageData = new vlByte[CVTFFile::ComputeImageSize(this->header->Width, this->header->Height, 1, IMAGE_FORMAT_RGBA8888)];
	vlByte *theThumbnailImageData = new vlByte[CVTFFile::ComputeImageSize(this->header->LowResImageWidth, this->header->LowResImageHeight, 1, IMAGE_FORMAT_RGBA8888)];

	if (!CVTFFile::ConvertToRGBA8888(this->GetData(0, 0, 0, 0), theImageData, this->header->Width, this->header->Height, this->header->ImageFormat)) {
		delete [] theImageData;
		delete [] theThumbnailImageData;

		return vlFalse;
	}
	if (!CVTFFile::Resize(theImageData, theThumbnailImageData, this->header->Width, this->header->Height, this->header->LowResImageWidth, this->header->LowResImageHeight)) {
		delete [] theImageData;
		delete [] theThumbnailImageData;

		return vlFalse;
	}
	if (!CVTFFile::ConvertFromRGBA8888(theThumbnailImageData, this->GetThumbnailData(), this->header->LowResImageWidth, this->header->LowResImageHeight, this->header->LowResImageFormat)) {
		delete [] theImageData;
		delete [] theThumbnailImageData;
...
vlBool CVTFFile::GenerateNormalMap(vlUInt uiFrame, VTFKernelFilter KernelFilter, VTFHeightConversionMethod HeightConversionMethod, VTFNormalAlphaResult NormalAlphaResult) {
	...
	if (this->header->Flags & TEXTUREFLAGS_ENVMAP) {
		LastError.Set("Image is an environment map.");
		return vlFalse;
	}
	if (this->imageData == 0) {
...
vlBool CVTFFile::GenerateSphereMap() {
	printf("CVTFFile::GenerateSphereMap()\n");
	if (!this->IsLoaded()) {
		return vlFalse;
	}
	if (!(this->header->Flags & TEXTUREFLAGS_ENVMAP)) {
		LastError.Set("Image is not an environment map.");
		return vlFalse;
	}
	if (this->header->StartFrame == 0xffff) {
		LastError.Set("Environment map does not have a sphere map.");
		return vlFalse;
	}
	if (this->imageData == 0) {
	...
	// lets go!
	vlByte *theImageData[6] = { 0, 0, 0, 0, 0, 0 };                                          // 6 pointers to memory for our faces.
	...
	for (i = 0; i < 6; i++) {
		...
		theImageData[j] = new vlByte[this->ComputeImageSize(uiWidth, uiHeight, 1, IMAGE_FORMAT_RGBA8888)];

		if (!this->ConvertToRGBA8888(this->GetData(0, i, 0, 0), theImageData[j], uiWidth, uiHeight, this->header->ImageFormat)) {
			for (vlUInt l = 0; l < 6; l++) {
				delete[] theImageData[l];
			}

			LastError.Set("Could not convert source to RGBA8888 format");
			return vlFalse;
		}
		SFace[j].buf = (vlUInt *)theImageData[j];        // save the address
	}
	...
	this->MirrorImage(theImageData[0], this->header->Width, this->header->Height);
	this->MirrorImage(theImageData[2], this->header->Width, this->header->Height);
	this->MirrorImage(theImageData[3], this->header->Width, this->header->Height);
	this->FlipImage(theImageData[4], this->header->Width, this->header->Height);
	this->FlipImage(theImageData[5], this->header->Width, this->header->Height);
	...
	vlByte *src = theImageData[3];   // 3 = up or forward face
	...
	for (vlUInt uiFrame = 0; uiFrame < uiFrameCount; uiFrame++) {
		for (vlUInt uiFace = 0; uiFace < uiFaceCount; uiFace++) {
			for (vlUInt uiSlice = 0; uiSlice < uiSliceCount; uiSlice++) {
				if (!this->ConvertToRGBA8888(this->GetData(uiFrame, uiFace, uiSlice, 0), theImageData, this->header->Width, this->header->Height, this->header->ImageFormat)) {
					delete [] theImageData;

					return vlFalse;
				}
				vlSingle sX, sY, sZ;
				this->ComputeImageReflectivity(theImageData, this->header->Width, this->header->Height, sX, sY, sZ);
...
vlUInt CVTFFile::ComputeDataOffset(vlUInt uiFrame, vlUInt uiFace, vlUInt uiSlice, vlUInt uiMipLevel, VTFImageFormat ImageFormat) const {
	...
	assert(uiOffset < this->imageDataLength);
...
vlBool CVTFFile::CompressDXTn(vlByte *lpSource, vlByte *lpDest, vlUInt uiWidth, vlUInt uiHeight, VTFImageFormat DestFormat) {
	...
	vlByte *rgbaBytes = lpSource;
	vlUInt byteLength = uiWidth * uiHeight * 4;
	vlByte *bgraBytes = new vlByte[byteLength];
...
vlBool CVTFFile::ConvertRGBA8888toBGRA8888(vlByte *src, vlByte *dest, vlUInt pixelCount) {
	printf("CVTFFile::ConvertRGBA8888toBGRA8888()\n");
	if (src == 0 || dest == 0 || pixelCount == 0) {
		LastError.Set("Invalid parameters to CVTFFile::ConvertRGBA8888toBGRA8888()");
		return vlFalse;
	}
	
//	vlUInt newLength = pixelCount * 4;
	
	int col;
	unsigned char *cp = (unsigned char *)&col;
	
	cp[3] = 0xff;	// default alpha if alpha isn't present
	
	TKColor32RGBA *pixels = (TKColor32RGBA *)src;
	TKColor32RGBA aPixel;
	
	for (vlUInt i = 0; i < pixelCount; i++) {
		aPixel = pixels[i];
		cp[0] = aPixel.b;
		cp[1] = aPixel.g;
		cp[2] = aPixel.r;
...
//vlByte *CVTFFile::CreateBGRADataFromColor32(Color32 *lpImagePixels, vlUInt pixelCount, vlUInt bitsPerPixel, vlUInt &length) {
//	if (lpImagePixels == 0 || pixelCount == 0 || bitsPerPixel == 0 || (bitsPerPixel != 24 && bitsPerPixel != 32) || length == 0) {
//		LastError.Set("Invalid parameters to CVTFFile::CreateBGRADataFromColor32()");
//		return 0;
//	}
//	
//	vlUInt newLength = pixelCount * 4;
...
//	
//	if (bytes == 0) {
//		LastError.Set("CVTFFile::CreateBGRADataFromColor32() malloc() failed!");
//		return 0;
//	}
//	
//	length = newLength;
...
//		pixel = lpImagePixels[i];
//		cp[0] = pixel.b;	/* set B component of col	*/
//		cp[1] = pixel.g;	/* set G component of col	*/
//		cp[2] = pixel.r;	/* set R component of col	*/
...
template<typename T>vlVoid GetShiftAndMask(const SVTFImageConvertInfo& Info, T &uiRShift, T &uiGShift, T &uiBShift, T &uiAShift, T &uiRMask, T &uiGMask, T &uiBMask, T &uiAMask) {
	if (Info.iR >= 0) {
...
template<typename T>T Shrink(T S, T SourceBits, T DestBits) {
...
// Upsample a channel.
template<typename T>T Expand(T S, T SourceBits, T DestBits) {
	if (SourceBits == 0 || DestBits == 0) {
		return 0;
	}
	T D = 0;
...
template<typename T, typename U>vlVoid Transform(TransformProc pTransform1, TransformProc pTransform2, T SR, T SG, T SB, T SA, T SRBits, T SGBits, T SBBits, T SABits, U& DR, U& DG, U& DB, U& DA, U DRBits, U DGBits, U DBBits, U DABits) {
...
template<typename T, typename U>vlBool ConvertTemplated(vlByte *lpSource, vlByte *lpDest, vlUInt uiWidth, vlUInt uiHeight, const SVTFImageConvertInfo& SourceInfo, const SVTFImageConvertInfo& DestInfo) {
...
#pragma mark -
#pragma mark ConvertToNormalMap()
...
//	Options.normalMap.bSignedOutput = KernelFilter == KERNEL_FILTER_DUDV;	// Normal maps seem to be signed in Source, DuDv maps don't.
	...
	inputOptions.setTextureLayout(TextureType_2D, uiWidth, uiHeight);
	inputOptions.setFormat(InputFormat_BGRA_8UB);
	inputOptions.setMipmapData(lpSourceRGBA8888, uiWidth, uiHeight);
	
	inputOptions.setNormalMap(false);
	inputOptions.setConvertToNormalMap(true);
    inputOptions.setHeightEvaluation(1.0f/3.0f, 1.0f/3.0f, 1.0f/3.0f, 4.0);
...
#endif
	LastError.Set("NVDXT or NVTT support required for CVTFFile::ConvertToNormalMap().");
	return vlFalse;
}
...
#endif
	LastError.Set("NVDXT or NVTT support required for CVTFFile::Resize().");
	return vlFalse;
}
...
vlVoid CVTFFile::FlipImage(vlByte *lpImageDataRGBA8888, vlUInt uiWidth, vlUInt uiHeight) {
	vlUInt *theImageData = (vlUInt *)lpImageDataRGBA8888;
	for (vlUInt i = 0; i < uiWidth; i++) {
		for (vlUInt j = 0; j < uiHeight / 2; j++) {
			vlUInt *pOne = theImageData + (i + j * uiWidth);
			vlUInt *pTwo = theImageData + (i + (uiHeight - j - 1) * uiWidth);
...
vlVoid CVTFFile::MirrorImage(vlByte *lpImageDataRGBA8888, vlUInt uiWidth, vlUInt uiHeight) {
	...
	for (vlUInt i = 0; i < uiWidth / 2; i++) {
		for (vlUInt j = 0; j < uiHeight; j++) {
			vlUInt *pOne = theImageData + (i + j * uiWidth);
			vlUInt *pTwo = theImageData + ((uiWidth - i - 1) + j * uiWidth);
