...
hlBool CDirectoryFolder::Extract(const hlChar *lpPath, hlChar **outPath) const {
	hlExtractItemStart(this);
	
	hlBool bResult;
	if (!CreateFolder(lpPath)) {
		LastError.SetSystemErrorMessage("CreateDirectory() failed.");
		...
		bResult = hlTrue;
		for (hlUInt i = 0; i < this->pDirectoryItemVector->size(); i++) {
			HLDirectoryItemType type = (*this->pDirectoryItemVector)[i]->GetType();
			if (type == HL_ITEM_FILE) {
				if (static_cast<const CDirectoryFile *> ((*this->pDirectoryItemVector)[i])->GetExtractable()) {
					bResult &= (*this->pDirectoryItemVector)[i]->Extract((const hlChar *)lpPath);
				}
			} else if (type == HL_ITEM_FOLDER) {
				bResult &= (*this->pDirectoryItemVector)[i]->Extract((const hlChar *)lpPath);
			}
		}
	}
	if (outPath) *outPath = strcpy(*outPath, lpPath);
