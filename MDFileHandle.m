...
enum {
	MDBrokenCreatorCode = 'MACS',
	MDBrokenFileType	= 'brok'
};


@interface MDFileHandle ()
@property (retain) NSString *path;
@end

@implementation MDFileHandle
...
- (id)initForWritingAtPath:(NSString *)aPath {
	if ((self = [super init])) {
		...
		if (!([fileManager fileExistsAtPath:aPath isDirectory:&isDir] && !isDir)) {
			...
			fInfo->fileType = MDBrokenFileType;
			fInfo->fileCreator = MDBrokenCreatorCode;
			fInfo->finderFlags = 0;
			Point point;
			point.v = 0;
			point.h = 0;
