/*
	File:       VSAuthorizationHelperToolLib.c
...
// Define VS_PRIVATE so that we pick up our private definitions from 
// "VSAuthorizationHelperToolLib.h".
...
// At runtime VSAuthorization only requires CoreFoundation.  However, at build time we need 
...
enum {
    kVSIdleTimeoutInSeconds			= 120,        // if we get no requests in 2 minutes, we quit
    kVSWatchdogTimeoutInSeconds		= 65          // any given request must be completely in 65 seconds
};
...
// kVSMaxNumberOfKBytes has two uses:
//
...
#define kVSMaxNumberOfKBytes			(1024 * 1024)
...
#define kVSSocketPathFormat			"/var/run/%s.socket"

// The key used to get our describe our socket in the launchd property list file.

#define kVSLaunchDSocketDictKey           "MasterSocket"
...
static Boolean VSIsBinaryPropertyListData(const void * plistBuffer, size_t plistSize)
...
{
    static const char kVSBinaryPlistWatermark[6] = "bplist";
    
    assert(plistBuffer != NULL);
	
	return (plistSize >= sizeof(kVSBinaryPlistWatermark)) 
        && (memcmp(plistBuffer, kVSBinaryPlistWatermark, sizeof(kVSBinaryPlistWatermark)) == 0);
}

static void VSNormalizeOSStatusErrorCode(OSStatus *errPtr) {
    // Normalize the cancelled error code to reduce the number of checks that our clients 
...
static int VSRead(int fd, void *buf, size_t bufSize, size_t *bytesRead) {
	...
	assert(buf != NULL);
    // bufSize may be 0
	assert(bufSize <= kVSMaxNumberOfKBytes);
...
static int VSWrite(int fd, const void *buf, size_t bufSize, size_t *bytesWritten) {
	...
	// which case you should define VS_WRITE_CHECK_SIGPIPE 
	// to 0 to bypass this check.
	
	#if !defined(VS_WRITE_CHECK_SIGPIPE)
		#define VS_WRITE_CHECK_SIGPIPE 1
	#endif
	#if !defined(NDEBUG) && VS_WRITE_CHECK_SIGPIPE
		{
			int					junk;
			struct stat			sb;
...
static int VSReadDictionary(int fdIn, CFDictionaryRef *dictPtr) {
	...
	//
	// See also the companion routine, VSWriteDictionary, below.
	...
	err = VSRead(fdIn, &dictSize, sizeof(dictSize), NULL);
	if (err == 0) {
		...
		if (dictSize == 0) {
			...
			// that case.
			err = EINVAL;
		} else if (dictSize > kVSMaxNumberOfKBytes) {
	...
	if (err == 0) {
		err = VSRead(fdIn, dictBuffer, dictSize, NULL);
	}
	if ( (err == 0) && VSIsBinaryPropertyListData(dictBuffer, dictSize) ) {
        err = VSOSStatusToErrno( coreFoundationUnknownErr );
	}
	if (err == 0) {
...
static int VSWriteDictionary(CFDictionaryRef dict, int fdOut) {
	// Write a dictionary to a file descriptor by flattening 
	// it into XML.  Send the size of the XML before sending 
	// the data so that VSReadDictionary knows how much to 
	// read.
	//
	// See also the companion routine, VSReadDictionary, above.
    ...
    if ( (err == 0) && (CFDataGetLength(dictData) > kVSMaxNumberOfKBytes) ) {
        err = EINVAL;
    }
    if (err == 0) {
		dictSize = OSSwapHostToBigInt32( CFDataGetLength(dictData) );
        err = VSWrite(fdOut, &dictSize, sizeof(dictSize), NULL);
    }
	if (err == 0) {
		err = VSWrite(fdOut, CFDataGetBytePtr(dictData), CFDataGetLength(dictData), NULL);
	}
...
static int VSReadDescriptor(int fd, int *fdRead) {
    ...
    if (err == 0) {
        err = VSWrite(fd, &kACKData, sizeof(kACKData), NULL);
        if (err != 0) {
            junk = close(*fdRead);
            assert(junk == 0);
            *fdRead = -1;
...
static int VSWriteDescriptor(int fd, int fdToWrite) {
    // Write the descriptor fdToWrite to fd.
    //
    // See the associated VSReadDescriptor, above.
    ...
    if (err == 0) {
        err = VSRead(fd, &ack, sizeof(ack), NULL);
...
extern void VSCloseDescriptorArray(CFArrayRef descArray) {
	// See comment in header.
	...
	//
	// VSCloseDescriptorArray((CFArrayRef) CFDictionaryGetValue(response, CFSTR(kVSDescriptorArrayKey)));
...
static int VSReadDictioanaryTranslatingDescriptors(int fd, CFDictionaryRef *dictPtr) {
    ...
    // closing any descriptors it references (VSCloseDescriptorArray makes 
	...
	err = VSReadDictionary(fd, &dict);
	
	// Now read the descriptors, if any.
	
	if (err == 0) {
		incomingDescs = (CFArrayRef)CFDictionaryGetValue(dict, CFSTR(kVSDescriptorArrayKey));
		...
		if (incomingDescs == NULL) {
			...
			*dictPtr = dict;
			...
			// kVSDescriptorArrayKey with translatedDesc.
			...
			if (err == 0) {
				...
				for (descIndex = 0; descIndex < descCount; descIndex++) {
					...
					err = VSReadDescriptor(fd, &thisDesc);
					if (err == 0) {
			...
			if (err == 0) {
				CFDictionarySetValue(mutableDict, CFSTR(kVSDescriptorArrayKey), translatedDescs);
				*dictPtr = mutableDict;
			} else {
				VSCloseDescriptorArray(translatedDescs);
                if (mutableDict != NULL) {
...
static int VSWriteDictionaryAndDescriptors(CFDictionaryRef dict, int fd) {
	...
	if (err == 0) {
		descArray = (CFArrayRef) CFDictionaryGetValue(dict, CFSTR(kVSDescriptorArrayKey));
		...
		if (descArray != NULL) {
			...
			if (err == 0) {
				...
				for (descIndex = 0; descIndex < descCount; descIndex++) {
					...
					if (err == 0) {
						err = VSWriteDescriptor(fd, thisDesc);
					}
...
static OSStatus VSFindCommand(
    ...
    commandStr = CFDictionaryGetValue(request, CFSTR(kVSCommandKey));
    ...
    if (retval == noErr) {
        do {
            ...
            if (commands[index].commandName == NULL) {
                retval = VSErrnoToOSStatus(ENOENT);
                break;
            }
        } while (true);
    }
...
/*
    Watchdog Timer
    --------------
    VSAuthorizationHelperToolLib's privileged helper tool server is single threaded.  Thus, 
    ...
    VSAuthorizationHelperToolLib code claims this signal and our clients are required not 
    to use it.  Also, the default disposition for SIGALRM is to quit the process, 
    which is exactly what I want.
*/

static void VSEnableWatchdog(void) {
    // Start the watchdog timer.  If you don't call VSDisableWatchdog before the 
    // timer expires, the process will die with a SIGALRM.
    (void) alarm(kVSWatchdogTimeoutInSeconds);
}


static void VSDisableWatchdog(void) {
    ...
    static bool VSCommandArraySizeMatchesCommandProcArraySize(
        const VSCommandSpec		commands[], 
...
/*
    ...
    ----------------------
    The on-the-'wire' protocol for a VSAuthorizationHelperToolLib connection (from the 
...
static int VSHandleConnection(
    ...
    assert( VSCommandArraySizeMatchesCommandProcArraySize(commands, commandProcs) );
    assert(fd >= 0);
    
    // Read in the external authorization reference.
    retval = VSRead(fd, &extAuth, sizeof(extAuth), NULL);
    
    // Internalize external authorization reference.
    if (retval == 0) {
        retval = VSOSStatusToErrno( AuthorizationCreateFromExternalForm(&extAuth, &auth) );
    }
    
    // Read in CFDictionaryRef request (the command and its arguments).
    if (retval == 0) {
        retval = VSReadDictionary(fd, &request);
    }
    ...
    if (retval == 0) {
        ...
        if (response == NULL) {
            retval = VSOSStatusToErrno( coreFoundationUnknownErr );
        }
    }

    // Errors that occur within this block are considered command errors, that is, they're 
    // reported to the client in the kVSErrorKey value of the response dictionary 
    // (that is, VSExecuteRequestInHelperTool returns noErr and valid response dictionary with 
    // an error value in the kVSErrorKey entry of the dictionary).  In contrast, other errors 
    // are considered IPC errors and generally result in a the client getting an error status 
    // back from VSExecuteRequestInHelperTool.
    //
    ...
    if (retval == 0) {        
        ...
        // not the command is valid by comparing with the VSCommandSpec array.  Also, 
        // if the command is valid, return the associated right (if any).

        commandProcStatus = VSFindCommand(request, commands, &commandIndex);
        ...
        if ( ! CFDictionaryContainsKey(response, CFSTR(kVSErrorKey)) ) {
            CFNumberRef     numRef;
            ...
            if (numRef == NULL) {
                ...
                CFDictionaryAddValue(response, CFSTR(kVSErrorKey), numRef);
                CFRelease(numRef);
            }
        }
    }
    ...
    if (retval == 0) {
        retval = VSWriteDictionaryAndDescriptors(response, fd);
    }
...
static void VSWaitForDebugger(aslclient asl, aslmsg aslMsg) {
	// You can force a debug version of the tool to stop and wait on 
	// launch using the following Terminal command:
	//
	// $ sudo launchctl stop com.markdouma.SourceFinagler
	// $ sudo launchctl setenv VSWaitForDebugger 1
	...
	value = getenv("VSWaitForDebugger");
...
static int VSCheckInWithLaunchd(aslclient asl, aslmsg aslMsg, const char **errStrPtr) {
	...
	fdArray = launch_data_dict_lookup(socketsDict, kVSLaunchDSocketDictKey);
	if (fdArray == NULL) {
...
static int VSSetNonBlocking(int fd, Boolean nonBlocking) {
...
extern int VSHelperToolMain(
    ...
    listener = VSCheckInWithLaunchd(asl, aslMsg, &errStr);
    if (listener < 0) {
        assert(errStr != NULL);
        goto done;
    }
    ...
    err = VSSetNonBlocking(listener, true);
    if (err != 0) {
        errno = err;            // for %m
    ...
    while (true) {
        ...
        static const struct timespec kIdleTimeout = { kVSIdleTimeoutInSeconds , 0 };
        ...
        if (thisConnection != -1) {
            ...
            thisConnectionError = VSErrnoToOSStatus( VSSetNonBlocking(thisConnection, false) );
            ...
            if (thisConnectionError == noErr) {
                thisConnectionError = VSHandleConnection(asl, aslMsg, commands, commandProcs, thisConnection);
            }
...
extern OSStatus VSExecuteRequestInHelperTool(
	...
	assert(CFDictionaryContainsKey(request, CFSTR(kVSCommandKey)));
	assert(CFGetTypeID(CFDictionaryGetValue(request, CFSTR(kVSCommandKey))) == CFStringGetTypeID());
    ...
    retval = VSFindCommand(request, commands, &commandIndex);

    #if !defined(VS_PREAUTHORIZE)
        #define VS_PREAUTHORIZE 1
    #endif
    #if VS_PREAUTHORIZE
    ...
    if (retval == noErr) {
        ...
        pathLen = snprintf(addr.sun_path, sizeof(addr.sun_path), kVSSocketPathFormat, bundleIDC);
	...
	if (retval == noErr) {	
		retval = VSErrnoToOSStatus( VSWrite(fd, &extAuth, sizeof(extAuth), NULL) );
	}
	
    // Write the request.
    
	if (retval == noErr) {	
		retval = VSErrnoToOSStatus( VSWriteDictionary(request, fd) );
	}
	
    // Read response, including any descriptors.
    
	if (retval == noErr) {
		retval = VSErrnoToOSStatus( VSReadDictioanaryTranslatingDescriptors(fd, response) );
    }
    ...
    VSNormalizeOSStatusErrorCode(&retval);
...
extern OSStatus VSGetErrorFromResponse(CFDictionaryRef response) {
	...
	num = (CFNumberRef) CFDictionaryGetValue(response, CFSTR(kVSErrorKey));
    err = noErr;
    ...
    VSNormalizeOSStatusErrorCode(&err);
	return err;
}
...
extern VSFailCode VSDiagnoseFailure(
    ...
    VSFailCode                 retval = kVSFailUnknown;
    int                         err;
    int                         pathLen;
    char                        bundleIDC   [ PATH_MAX ];
    ...
    if ( CFStringGetFileSystemRepresentation(bundleID, bundleIDC, sizeof(bundleIDC)) ) {
        ...
        pathLen = snprintf(toolPath,  sizeof(toolPath),  kVSToolPathFormat,  bundleIDC);
        assert(pathLen < PATH_MAX);         // snprintf truncated the string; won't crash us, but we want to know

        pathLen = snprintf(plistPath, sizeof(plistPath), kVSPlistPathFormat, bundleIDC);
        assert(pathLen < PATH_MAX);         // snprintf truncated the string; won't crash us, but we want to know
        ...
        if ( (toolErr == 0) && (plistErr == 0) ) {
            ...
            if (fd != -1) { 
                ...
                addr.sun_family = AF_UNIX;
                (void) snprintf(addr.sun_path, sizeof(addr.sun_path), kVSSocketPathFormat, bundleIDC);
				addr.sun_len    = SUN_LEN(&addr);
            ...
            if ( (toolErr == 0) || (plistErr == 0) ) {
                retval = kVSFailPartiallyInstalled;
            } else {
                retval = kVSFailNotInstalled;
            }
        }
    }
...
static const char * kPlistTemplate =
    ...
    //
    // IMPORTANT
    // kVSToolPathFormat embeds a %s

    "	<key>ProgramArguments</key>\n"
    "	<array>\n"
    "		<string>" kVSToolPathFormat "</string>\n"
    "	</array>\n"
    ...
    //
    // IMPORTANT
    // kVSSocketPathFormat embeds a %s

    "	<key>Sockets</key>\n"
    "	<dict>\n"
    "		<key>" kVSLaunchDSocketDictKey "</key>\n"
    "		<dict>\n"
    "			<key>SockFamily</key>\n"
    "			<string>Unix</string>\n"
    ...
    "			<string>" kVSSocketPathFormat "</string>\n"
    "			<key>SockType</key>\n"
...
//  pretty distinctive text to make it clear that we've got the right thing.
    ...
    static Boolean gVSLogInteractions = false;
        // Set gVSLogInteractions to have VSFixFailure log its interactions with 
        // the installation tool to stderr.

    static Boolean gVSLogInteractionsInitialized = false;
        // This indicates whether we've initialised gVSLogInteractions from the 
		// environment variable.

#endif

static OSStatus VSRunInstallToolAsRoot(
	AuthorizationRef			auth, 
    const char *                installToolPath,
	const char *				command, 
								...) {
    // Run the specified install tool as root.  The arguments to the tool are 
    // given as a sequence of (char *)s, terminated by a NULL.  The tool is 
    ...
    if (retval == noErr) {
        #if !defined(NDEBUG)
			if ( ! gVSLogInteractionsInitialized ) {
				const char *	value;
				
				value = getenv("VSLogInteractions");
				gVSLogInteractions = ( ((value != NULL) && (atoi(value) != 0)) );
				
				gVSLogInteractionsInitialized = true;
			}
		
            if (gVSLogInteractions) {
                argIndex = 0;
    ...
    if (retval == noErr) {
        ...
        do {
			...
			if (strcmp(thisLine, kVSInstallToolSuccess "\n") == 0) {
                assert(retval == noErr);
				break;
			}
            ...
            if ( sscanf(thisLine, kVSInstallToolFailure "\n", &tmpInt) == 1 ) {
                retval = VSErrnoToOSStatus( tmpInt );
                if (retval == noErr) {
                    assert(false);
                    retval = errState;
                }
			...
			if ( (childPID == -1) && (sscanf(thisLine, kVSAntiZombiePIDToken1 "%ld" kVSAntiZombiePIDToken2 "\n", &tmpLong) == 1) ) {
...
static OSStatus VSInstall(
    ...
    if (retval == noErr) {
        strlcpy(plistPath, "/tmp/VSTemp-XXXXXXXX.plist", sizeof(plistPath));
    ...
    if (retval == noErr) {
        retval = VSErrnoToOSStatus( VSWrite(fd, plistText, strlen(plistText), NULL) );
    }
    
    // Run the tool as root using AuthorizationExecuteWithPrivileges.
    
    if (retval == noErr) {
        retval = VSRunInstallToolAsRoot(auth, installToolPath, kVSInstallToolInstallCommand, bundleID, helperToolPath, plistPath, NULL);
    }
...
static OSStatus VSGetToolPath(CFStringRef bundleID, CFStringRef toolName, char *toolPath, size_t toolPathSize) {
...
extern OSStatus VSFixFailure(
    ...
    if (retval == noErr) {
        retval = VSGetToolPath(bundleID, installToolName, installToolPath, sizeof(installToolPath));
    }
    if (retval == noErr) {
        retval = VSGetToolPath(bundleID, helperToolName,  helperToolPath,  sizeof(helperToolPath));
    }
    ...
    if (retval == noErr) {
        if (failCode == kVSFailDisabled) {
            retval = VSRunInstallToolAsRoot(auth, installToolPath, kVSInstallToolEnableCommand, bundleIDC, NULL);
        } else {
            retval = VSInstall(auth, bundleIDC, installToolPath, helperToolPath);
        }
    }
